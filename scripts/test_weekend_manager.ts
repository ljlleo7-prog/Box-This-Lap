...
console.log('Allocations (returned):', weekend.tyreAllocations['driver1'].filter(t => t.returned).length); // 2

// fp2 -> fp3
weekend = WeekendManager.transitionToNextPhase(weekend);
console.log('Phase:', weekend.currentPhase); // fp3
console.log('Allocations (returned):', weekend.tyreAllocations['driver1'].filter(t => t.returned).length); // 4

// fp3 -> q1 (Parc Ferme)
weekend = WeekendManager.transitionToNextPhase(weekend);
console.log('Phase:', weekend.currentPhase); // q1
console.log('Allocations (returned):', weekend.tyreAllocations['driver1'].filter(t => t.returned).length); // 6
...
console.log('Allocations (returned):', weekend.tyreAllocations['driver1'].filter(t => t.returned).length); // 7 (6 + 1 from q3)
