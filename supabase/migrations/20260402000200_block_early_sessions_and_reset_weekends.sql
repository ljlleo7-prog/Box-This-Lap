...
BEGIN
  ...
  IF v_weekend.scheduled_practice_at_utc IS NOT NULL AND NOW() < v_weekend.scheduled_practice_at_utc THEN
    RETURN jsonb_build_object('success', false, 'message', 'Practice cannot be run before its scheduled time');
  END IF;
...
BEGIN
  ...
  IF v_weekend.scheduled_quali_at_utc IS NOT NULL AND NOW() < v_weekend.scheduled_quali_at_utc THEN
    RETURN jsonb_build_object('success', false, 'message', 'Qualifying cannot be run before its scheduled time');
  END IF;
...
BEGIN
  ...
  IF v_weekend.scheduled_race_at_utc IS NOT NULL AND NOW() < v_weekend.scheduled_race_at_utc THEN
    RETURN jsonb_build_object('success', false, 'message', 'Race cannot be run before its scheduled time');
  END IF;
