import { RaceState, VehicleState, Track, Driver } from '../../types/index.ts';
import { SeededRNG } from '../rng.ts';
import { StrategySystem } from './StrategySystem.ts';
...
export class RaceLogicSystem {
  ...
  public initializeRace(track: Track, drivers: Driver[], strategySystem: StrategySystem, gridOrder?: string[]): RaceState {
    ...
    // Qualifying / Grid Order Logic
    let sortedDrivers: Driver[] = [];
    
    if (gridOrder && gridOrder.length > 0) {
        // Use provided grid order
        sortedDrivers = gridOrder
            .map(id => drivers.find(d => d.id === id))
            .filter((d): d is Driver => !!d);
            
        // Append any drivers not in grid order at the back
        const missing = drivers.filter(d => !gridOrder.includes(d.id));
        sortedDrivers.push(...missing);
    } else {
        ...
        sortedDrivers = qualifyingResults.map(r => r.driver);
    }

    const vehicles: VehicleState[] = sortedDrivers.map((driver, index) => {
