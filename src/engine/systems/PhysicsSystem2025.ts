...
export class PhysicsSystem2025 implements IPhysicsSystem {
  ...
  public updateVehiclePhysics(vehicle: VehicleState, driver: Driver, state: RaceState, track: Track, trackProfile: TrackProfile, dt: number, teamSpecs?: TeamSpecs): void {
      ...
      const profileDynamics = this.getProfileDynamics(vehicle, profileSample, trackProfile, track, driver);
      ...
      const isHotlap = vehicle.paceMode === 'aggressive';
      ...
      // Increase braking baseline significantly to remove overshoot
      const maxBrake = this.calculateMaxBraking(vehicle.speed, sectorType, profileDynamics) * gripFactor * brakeFactor * driverBrakeFactor * 1.5;

      // ---- GEOMETRIC REVERSE ENGINEERING ----
      // Reverse-engineer track curvature from telemetry to allow natural acceleration based on the traction circle
      // We look ahead slightly to get the track's target speed gradient
      const v_tel = Math.max(1, lookupTrackProfileSpeed(trackProfile, vehicle.distanceOnLap));
      const v_tel_next = lookupTrackProfileSpeed(trackProfile, vehicle.distanceOnLap + 1);
      
      // Allow aggressive punch out if the track is opening up (dv/ds > 0)
      let dv_ds = (v_tel_next - v_tel) / 1.0;
      
      // If we are artificially slower than the telemetry (e.g. driver mistake or traffic),
      // we can accelerate harder because the corner curvature allows the higher telemetry speed.
      if (vehicle.speed < v_tel) {
          dv_ds = Math.max(dv_ds, (v_tel - vehicle.speed) / 5.0); // Recover lost speed aggressively
      }

      const a_lon_tel = v_tel * dv_ds;

      // Base grip of the OpenF1 reference car (approx 1.5G mechanical + aero)
      // We use a generous grip model here because we want to allow the car to punch the throttle
      // when the corner is opening up.
      const getGrip = (v: number, multi: number) => (18.0 * multi) + (0.003 * v * v);
      const G_tel = getGrip(v_tel, 1.0);

      // Clamp to prevent NaN if telemetry has an anomaly
      const a_lon_tel_clamped = Math.max(-G_tel + 0.1, Math.min(G_tel - 0.1, a_lon_tel));
      const a_lat_tel = Math.sqrt(Math.max(0, G_tel * G_tel - a_lon_tel_clamped * a_lon_tel_clamped));
      ...
      const G_sim = getGrip(vehicle.speed, overallGripMultiplier);
      const a_lat_sim = curvature * vehicle.speed * vehicle.speed;

      let max_a_lon_grip = 0;
      if (G_sim >= a_lat_sim) {
          max_a_lon_grip = Math.sqrt(G_sim * G_sim - a_lat_sim * a_lat_sim);
      } else {
          // Exceeding lateral grip limit, need to lift/brake to regain grip
          max_a_lon_grip = -Math.sqrt(a_lat_sim * a_lat_sim - G_sim * G_sim);
      }
      // ---------------------------------------
      ...
      // The engine's maximum allowed acceleration is bounded by available longitudinal grip
      const gripLimitedAccel = Math.min(accelRate, max_a_lon_grip);
      ...
      let desiredAccel = gripLimitedAccel;
      ...
      if (profileDynamics.needsBrakingNow) {
          ...
          // Apply an immediate braking boost if we are severely overshooting the target speed
          // This stops the car from bleeding too deep into the apex
          if (vehicle.speed > approachTargetSpeed + 5) {
             desiredAccel *= 2.0; // Brake 100% harder if overshooting
          }
      } else {
          // If we are way over the raw target speed and losing grip, apply brakes
          if (vehicle.speed > targetSpeed + 2 && max_a_lon_grip < 0) {
              desiredAccel = Math.max(-brakeRate, max_a_lon_grip * 2.5); // Apply progressive negative acceleration based on grip deficit
          } else {
              // Freely accelerate out of the corner, constrained only by physical traction!
              desiredAccel = gripLimitedAccel;
          }
      }
  ...
  private getProfileDynamics(vehicle: VehicleState, profileSample: TrackProfileSample, trackProfile: TrackProfile, track: Track, driver: Driver): any {
    ...
    const driverAggression = driver.personality?.aggression ?? 50;
    
    // actualBrakePoint is how far ahead we actually want to start braking
    // Add a safety buffer to start braking slightly earlier
    // Aggressive drivers brake later (smaller buffer)
    const aggressionBufferOffset = ((driverAggression - 50) / 50) * 10; // -10m to +10m based on aggression
    const actualBrakePoint = projectedBrakeNeed + brakeShiftMeters + 35 - aggressionBufferOffset;
  ...
  private calculateTargetSpeed(vehicle: VehicleState, driver: Driver, state: RaceState, track: Track, trackProfile: TrackProfile, profileSample: TrackProfileSample, profileDynamics: any, teamSpecs?: TeamSpecs, setupEffects = buildSetupPhysicsEffects(track, vehicle, teamSpecs), includeTraffic: boolean = true): number {
    ...
    // Add consistency variance - drivers with lower consistency fluctuate more from their target pace
    const consistency = driver.skill?.consistency ?? 50;
    const consistencyVariance = (100 - consistency) * 0.0002;
    // We use a simple pseudo-random fluctuation based on the distance
    const fluctuation = Math.sin(vehicle.distanceOnLap * 0.01) * consistencyVariance;
    const finalDriverFactor = driverFactor + fluctuation;
    ...
    // If vehicle is in dirty air, they lose a significant chunk of downforce (grip)
    // This affects cornering speeds (which are derived from the overallGripMultiplier)
    let dirtyAirPenalty = 1.0;
    if (vehicle.inDirtyAir && !isStraight) {
        if (sectorType === 'corner_high_speed') {
            dirtyAirPenalty = 0.96; // 4% speed loss in high-speed corners (aero dependent)
        } else if (sectorType === 'corner_medium_speed') {
            dirtyAirPenalty = 0.98; // 2% speed loss in medium corners
        } else {
            dirtyAirPenalty = 0.99; // 1% speed loss in slow corners (mechanical grip dominant)
        }
    }
    ...
    if (isStraight && !profileDynamics.needsBrakingNow) {
       ...
       speed *= finalDriverFactor * teamFactor * scaledTyrePerformance * scaledFuelFactor * paceFactor * lineFactor * moraleFactor * formFactor * temperatureFactor * difficultyFactor * dirtyAirPenalty * setupPhaseFactor * (1 - rotationPenalty);
    ...
    if (includeTraffic && vehicle.position > 1 && state.currentLap > 1) {
      // Use physical gap instead of standings gap for traffic logic
      ...
      const gapTime = Math.max(0.1, physicalGapDist / smoothedSpeed);
      
      if (isStraight && gapTime < 1.5) {
        speed *= 1 + 0.04 * Math.max(0, 1 - (gapTime / 1.5));
      } else if (!isStraight && gapTime < 2.0) {
        const penaltyBase = sectorType === 'corner_high_speed' ? 0.035 : sectorType === 'corner_medium_speed' ? 0.025 : 0.015;
        speed *= 1 - penaltyBase * Math.max(0, 1 - (gapTime / 2.0));
      }
    }

    if (includeTraffic && vehicle.isBattling && state.safetyCar === 'none') {
      const aheadId = vehicle.executionState.physicalAheadId;
      const ahead = aheadId ? state.vehicles.find(v => v.id === aheadId) : null;
      if (ahead) {
  ...
  private calculateMaxAcceleration(speed: number, driver: Driver, drsOpen: boolean = false, gapToAhead: number = 100, sectorType: string = 'straight', ersMode: VehicleState['ersMode'] = 'balanced', ersLevel: number = 100, ersOverrideEligible: boolean = false, teamSpecs?: TeamSpecs, powerUnitPhilosophy: VehicleState['powerUnitPhilosophy'] = 'balanced', batteryAllocationMode: VehicleState['batteryAllocationMode'] = 'balanced', ersTacticalState?: ERSTacticalState, profileDynamics?: any): number {
      ...
      const batteryHarvestPenalty = batteryAllocationMode === 'attack'
        ...
        : batteryAllocationMode === 'conservative'
          ? 1.06
          : 1;
      const aeroDragFactor = 1.0; // 2025 cars do not have active aero (except DRS, handled later)
      const efficiencyFactor = (0.98 + (ersEfficiency - 85) / 500) * batteryHarvestPenalty;
      
      // Check ERS deployment limit tapering which might have been giving weird boosts
      let deploymentLimit = this.getERSDeploymentLimit(speedKph, ersMode, efficiencyFactor, ersOverrideEligible) * batteryDeployFactor;
      
      // Standardize high speed ERS dropoff to prevent late straight surges
      // 2025 ERS doesn't have a strict tapering rule, just a soft max speed
      if (speedKph > 330 && !ersOverrideEligible) {
          deploymentLimit = 0;
      }
      
      // 2025 ERS Limits: 4MJ battery, 4MJ deploy per lap, 2MJ harvest per lap
      const maxBattery = 4000000;
      const batteryEnergy = (ersLevel / 100) * maxBattery;
      
      // Enforce battery state limits
      deploymentLimit = Math.min(
          deploymentLimit, 
          batteryEnergy / 0.1 // Max instantaneous power from remaining battery
      );
      
      // In 2025, ERS is typically deployed heavily on straights and slightly on corner exits.
       // Do not deploy in corner entry or mid-corner where traction is limited and engine braking is needed.
       if (sectorType !== 'straight') {
           if (profileDynamics?.phase === 'exit') {
               // Allow full ERS deployment, it will be naturally bounded by the physical traction circle limit!
               deploymentLimit *= 1.0; 
           } else {
               deploymentLimit = 0; // No ERS deployment during braking/apex
           }
       }
      ...
      const effectiveSpeed = Math.max(5, speed); // Reduced from 15 to allow massive torque spike at low speeds
      let thrustForce = availablePower / effectiveSpeed;
      const driverTractionSkill = driver.skill?.tyreManagement ?? 50; // Smooth throttle application
      const driverThrottleFactor = 1.0 + ((driverTractionSkill - 50) / 100) * 0.10; // ±10% better traction usage

      // On corner exits, the traction limit is less restrictive as the car straightens up
      const tractionLimitForce = mass * gravity * (profileDynamics?.phase === 'exit' ? 2.5 : (profileDynamics?.tractionFactor ? (1.50 + Math.max(0, profileDynamics.tractionFactor - 1) * 1.1) : 1.50)) * driverThrottleFactor;
    ...
    // Overtake/Deploy boost shouldn't cause an artificial drag-reduction at high speeds. 
    // The previous math was creating an inverted drag curve if the gap/slipstream logic overlapped wrong.
    // Slipstream provides a massive drag reduction if you are close behind another car
    const maxSlipstream = drsOpen ? 0.08 : 0.15; // Increased slipstream effect (up to 15% drag reduction)
    
    // Slipstream is effective up to 1.5 seconds behind, but scales linearly
    if (sectorType === 'straight' && gapToAhead < 1.5) {
        const slipstreamFactor = Math.max(0, 1 - (gapToAhead / 1.5));
        dragArea *= (1 - (maxSlipstream * slipstreamFactor));
    }
    
    // Add an aggressive speed-dependent drag multiplier at very high speeds to enforce the wall-of-air effect
    if (speed > 75) { // ~270 km/h
        // Increase exponent to create a stronger "wall" effect and prevent acceleration past ~330km/h
        const highSpeedPenalty = 1 + Math.pow((speed - 75) / 10, 2.0) * 0.08;
        dragArea *= highSpeedPenalty;
    }
  ...
  private getERSDeploymentLimit(speedKph: number, mode: VehicleState['ersMode'], efficiencyFactor: number, _ersOverrideEligible: boolean): number {
      const maxPower = 120000;
  ...
  private getERSHarvestPower(speedKph: number, mode: VehicleState['ersMode'], efficiencyFactor: number): number {
      const baseHarvest = 60000;
      if (speedKph < 80) return 0;
      // In 2025, MGU-K can harvest max 2MJ per lap, MGU-H is unlimited but functionally capped by exhaust
      switch (mode as string) {
